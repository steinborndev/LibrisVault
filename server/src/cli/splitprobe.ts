...
function main(): number {
  ...
  if (flag('--stability') && base.shelves.length > 0) {
    ...
    for (let k = 1; k <= 5; k++) {
      ...
      const d = shelfDrift(base, other)
      console.log(`  seed +${k}        shelves ${String(other.shelves.length).padStart(2)}  moved ${String(d.moved).padStart(3)} of ${d.common}  ${pct(d.share)}`)
    }
    const now = Date.now()
    for (const days of [3, 14, 30]) {
      const cut = now - days * DAY_MS
      const past = restrictGraph(graph, (n) => n.freshMs === undefined || n.freshMs <= cut)
      const then = proposeSplit(past, domain, addresses)
      const d = shelfDrift(base, then)
      console.log(
        `  ${String(days).padStart(2)} days back   shelves ${String(then.shelves.length).padStart(2)}  pages ${then.pages}  moved ${String(d.moved).padStart(3)} of ${d.common}  ${pct(d.share)}`,
      )
    }
  }
  return 0
