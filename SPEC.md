...
**Surface:** `GET /api/v1/health` reports `demoMode: true`. The dashboard shows a read-only notice instead of the setup banner; System renders read-only (since 2026-09-25, it used to show an explanation surface): every section draws from the same reads as on a full instance - the maintenance areas with their state, the standing defects, the split proposal, usage, vault stats, history, the settings - and every action in it is shown disabled, with one line under the head saying why; the credential card says the demo needs none rather than reporting it missing; Research renders read-only (since 2026-09-18, it used to show the same explanation surface): the saved conversations with their citations and the finished research runs can be browsed, the composer is disabled with a hint, and nothing can be started from it; intake surfaces (dropzone, global drag-and-drop) are gone. In demo mode `GET /api/v1/settings` names neither vault path nor watch folder nor bind address, and `GET /api/v1/stats` does not name the watch folder either (it did until 2026-09-25, found when System became browsable); a public instance has no reason to reveal its file-system layout.
