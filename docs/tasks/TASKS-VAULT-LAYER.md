...
| B8 | 47 upstream demo pages, 25 with the community footer | **13 content pages marked `origin: upstream-demo` and every reader honours it**: counters 1208 to **1195**, the index gives them their own section, retrieval never offers them, the graph and the catalog put them behind the System toggle. 13 and not 47 because the mark needs BOTH conditions - created before the vault took real material AND carrying the upstream footer - and because four pages the SERVICE writes had been marked by mistake | 8.7 |
...
### 2.7 The `_index.md` bucket hubs - DONE 2026-09-19 (and the plan for them was wrong)

**Read this before touching these three files again.** The plan called them dated event
sections and said to drop them and generate a page list in their place. Measured before doing
it: the three hubs hold **1129 entries and every single one carries a hand-written one-line
description**, covering 595 of 604 concepts, 327 of 338 sources, 206 of 225 entities. The text
exists nowhere else. Deleting it would have been the largest content loss of this phase, and it
is the SECOND time in it that a "dated section" turned out to be the content - the first was
`overview.md`, where it really was a duplicated activity log. The heading shape is not evidence.

So the defect is the ORGANISATION, not the content: the hub is grouped by the ingest that
happened to write each entry, which is the vault's history rather than its subject. A reader
looking for what the vault holds on a topic had to know when it arrived.

**`bucketRegroupPass` re-files every entry verbatim under its page's own `domain:`**, in the
same order the index uses. 394 headings become 61. The 39 pages the hubs never listed are
appended with no description - inventing one is what this pass may not do, and a visible gap
beats a silent one. A page listed twice keeps its longer entry, because both were written by
runs, neither is authoritative, and "whichever came first" is a coin toss dressed as a rule.
Verified before applying: 0 pages lost, one line dropped (the shorter half of the one
duplicate), each file's head byte-identical.

| hub | before | after | headings |
|---|---|---|---|
| `concepts/_index.md` | 149 kB | 140 kB | 135 to 21 |
| `sources/_index.md` | 120 kB | 109 kB | 169 to 20 |
| `entities/_index.md` | 37 kB | 32 kB | 90 to 20 |

**And a consequence for the writer, which is the real conclusion of this task.**
`updateBucketHub` may no longer create a generated block in a hub that already lists pages: it
would write all 604 entries a second time, bare, beside the annotated ones. That is the same
contract as everywhere else, read the other way round - the service owns the region between the
markers, and here **there is nothing for it to own**, because what makes these pages worth
reading is exactly the part no generator can produce. They stay agent-owned; the regroup pass
is what appends whatever a run forgot.

### 2.7 The mechanism, as originally recorded
...
- [x] `_index.md` hubs per 2.7, with the curated descriptions preserved. **DONE 2026-09-19,
      and "preserved" turned out to be the whole task** - see 2.7's result below. 394 headings
      to 61, all 1129 descriptions kept word for word, 39 unlisted pages added, 0 pages lost.
      Vault commit `41bc8b01`.
...
- [x] 231 occurrences over 96 targets, **now 87 over 38** (vault commit `e92444fd`) - but NOT
      the way this bullet says, and the difference is a correction to the plan.
      **"Repair both ends" assumes the file name was DERIVED from the title. It was not.** Of
      the 42 colon cases the best single transformation explains 7; dropping the colon entirely
      explains 9; `"/" -> "_"` explains 3 of 14 slash cases. Nothing explains the rest, because
      a run chose a name and separately chose a title, and a file name is often a deliberately
      shorter one. There is no rule to apply, and `title := file name` would throw away what
      the title says.
      So only the link end is repaired, and it is the end that costs something: of 61 drifted
      pages **only 11 are linked from anywhere at all**. Those links now read
...
- [x] **DONE 2026-09-19.** Type mirrors: 1043 assignments removed, mirroring 0 % in every
      month (vault commit `ad7b099`). Singletons: 274 removed over 230 pages (vault commit
      `4bccb7c7`), 322 to 48.
      **"Merge the obvious variants" had nothing to merge**, and that measurement is what
      decided the shape: of the 322 tags used exactly once, **none is a spelling variant of a
      tag used elsewhere**. They are genuinely one-off names, so the choice was keep or drop,
      not merge. A tag naming one page groups nothing; it is a second title in the tag field.
      **48 remain on purpose.** The first run dropped all 322 and emptied the tag block on 36
      pages whose every tag was a one-off. The validator's `frontmatter` findings went 9 to 45
      - and that rule is not ours, it mirrors the VAULT's own page template. The pass had not
      found 36 problems, it had made them. Reverted whole, guarded, re-run: a page keeps tags
      that group nothing rather than violating the template, because picking a replacement
      would be inventing the taxonomy this pass may not invent.
...
- [x] **DONE 2026-09-19, by moving rather than removing** (vault commit `665ded4`).
      `runProtocolPass` had removed the 14 sections that were short and cited nothing, which is
      boilerplate by any reading, and left 273 over eight headings. Those were left BECAUSE
      they carry arguments - a caveat about a source, a reason a claim is hedged, a paragraph
      distinguishing two papers - and deleting them is the prose rewrite this phase forbids.
      So they moved: **257 pages, every section now under one `## About This Page` at the foot,
      keeping its own name one level down.** Every word kept, page size barely moves, and the
      article reads through again. `run-protocol` findings 217 to 0.
      **`## Open Questions` is excluded and must stay excluded.** It is a live feature:
      `candidates.ts` reads that section BY NAME to plan a Fellow's work and
      `POST /questions/archive` strikes through its bullets. Moving it under another heading
      breaks the planner silently, which is the same shape of mistake as correction C-1.
      Original bullet, for the record: 352 pages, 302 kB, the seven relocated headings
