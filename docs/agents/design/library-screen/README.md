...
- `Main.dc.html` day view, `Night.dc.html` night shift, `Focus.dc.html` focus mode with the
  Fellow popover, `Card.dc.html` docked Fellow card, `Textures.dc.html` floor, wall and shelf
  options with three presets, `Sprites.dc.html` the style A pose vocabulary (round 2; the
