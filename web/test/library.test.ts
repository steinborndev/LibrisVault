...
describe('scene adapter', () => {
  ...
  it('places Fellows by state and pose: shelf in the home domain\'s room, desk in the main room, armchairs when resting', () => {
    ...
    expect(byName['Cy']).toMatchObject({ pose: 'sleep', tag: 'warn', caption: 'Cy · quota spent' })
    expect(byName['Di']).toMatchObject({ pose: 'sit', tag: 'fellow', caption: 'Di · plan for tonight' })
