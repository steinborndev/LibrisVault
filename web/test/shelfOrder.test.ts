...
import { orderedDomains, stepInOrder } from '../src/lib/library/shelfOrder.ts'
...
describe('stepping to the next one', () => {
  ...
  it('moves one along in either direction', () => {
    expect(stepInOrder(order, 'a', 1)).toBe('b')
    expect(stepInOrder(order, 'b', -1)).toBe('a')
  })

  it('wraps at both ends, because a ring of shelves has no dead end', () => {
    expect(stepInOrder(order, 'c', 1)).toBe('a')
    expect(stepInOrder(order, 'a', -1)).toBe('c')
  })

  it('starts somewhere sensible when the current shelf is not in the list', () => {
    expect(stepInOrder(order, null, 1)).toBe('a')
    expect(stepInOrder(order, null, -1)).toBe('c')
    expect(stepInOrder(order, 'deleted', 1)).toBe('a')
  })

  it('goes nowhere when there is nowhere to go', () => {
    expect(stepInOrder([], 'a', 1)).toBeNull()
    expect(stepInOrder(['a'], 'a', 1)).toBeNull()
    expect(stepInOrder(['a'], 'a', -1)).toBeNull()
  })
})
